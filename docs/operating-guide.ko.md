...
### 운영자 신용과 재입고 (기록이 담보를 대신한다)

키오스크 하단 **운영자 신용 / 재입고** 섹션. 별도 env 없이 항상 켜져
있고, 재입고 일감 게시만 `MACHINE_LOCATION`을 요구한다.

**신용 점수(0~100)** 는 부스가 실제로 기록한 것에서만 나온다 — 판매,
매진(손님이 왔는데 못 판 것), 재입고와 그 대응 속도, 운영 기간.
백필은 없다: 원장이 생긴 순간부터가 그 사람의 역사고, 기록 5건 미만은
**미평가·0점**이다. "판매 1건, 매진 0건"이 100점이 되는 순간 이 숫자는
거짓말이 되니까.

| 구성 | 배점 | 의미 |
|---|---|---|
| 충족률 | 45 | 판매 ÷ (판매+매진). 매진은 불운이 아니라 운영자가 안 한 일이다 |
| 재입고 대응 | 25 | **한 번도 안 비운 게 만점.** 24시간 내 대응 만점, 일주일이면 0 |
| 운영 기간 | 15 | 90일에 만점 |
| 누적 판매 | 15 | 로그 스케일 — 10번째 판매가 110번째보다 크게 잡힌다 |

**실제로 집행되는 건 유보 보증금이다.** 선불 담보는 견적만 보여주고
받지 않는다 (받는 흐름이 없는데 받는 척하는 게 제일 나쁜 기능이라서).
대신 운영자 **수익에서** 유보한다: 미평가 30% → 우수 5%, 점수가 오르면
자동으로 풀려 지급된다. 자본 없이 시작할 수 있으면서 실제로 잃을 것이
생기고, 그게 operatorship의 네 번째 조건(downside)을 진짜로 만든다.
`/operators`에 유보액과 점수 산출 근거가 그대로 나온다.

**재입고**: 재고 1개 이하면 목록에 뜬다. **이미 손님을 돌려보낸** 슬롯이
아직 아무도 안 온 빈 슬롯보다 위로 정렬된다. 운영자가 직접 채웠으면 수량만
등록하고, 자리에 없으면 **기계가 사람을 고용한다** —
`MACHINE_LOCATION`이 있으면 "이 일감 시장에 올리기" 버튼이 뜨고,
임차인의 **적립 수익에서** 바운티가 나가는 실제 에스크로 잡이 Handsel에
게시된다. 부스 돈이 아니다: 부스가 대신 내주면 그건 기계 주인이 임차인
일을 공짜로 해주는 것이고, operatorship이 없애려는 구조 그 자체다.
적립이 0이면 고용 못 하고 이유가 그대로 표시된다. (게시는 버튼으로만 —
타이머로 돌리면 감지 버그 하나가 수수료를 반복 지출한다. 같은 슬롯의
열린 바운티가 이미 있으면 중복 게시도 막는다.)

**증거 등급**: 재입고 등록은 처음엔 `self-reported`다. 그 슬롯에서
**다음 유료 배출이 성공하면** 자동으로 `confirmed-by-sale`로 올라간다 —
빈 매거진은 팔 수 없으니, 기계가 평소 일을 한 것 자체가 증명이 된다.
센서도 사진도 필요 없고, 확인된 재입고가 점수에서 더 높게 쳐진다.
(배출구에 IR 센서를 달면 `instrumented`로 한 단계 더 — 아직 미배선.)
...
| 신용·재입고 | `OPERATOR_EVENTS_FILE`, `MACHINE_LOCATION` | 신용은 항상 ON, 일감 게시만 OFF |
